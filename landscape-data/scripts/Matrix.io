...
Matrix := Object clone do(
  ...
  set := method(
    ...
    for (c,0,columns-1, for (r,0,rows-1,
      entries add(args at (r*columns+c))
    ))
    self
  )
...
vector := block(
  M := Matrix clone dim(thisMessage arguments count,1)
  M setList(thisMessage argsEvaluatedIn(sender) )
)

matrix := block(
  rows := List clone add(List clone)
  maxcols := 1
  thisMessage arguments foreach(i, arg,
    while(arg nextMessage,
      arg0 := arg clone
      arg0 setNextMessage(Nil)
      rows last add( sender doMessage(arg0) )
      if (rows last count > maxcols, maxcols = rows last count)
      rows add(List clone)
      arg = arg nextMessage
    )
    arg0 := arg clone
  ...
  M := Matrix clone dim(rows count, maxcols)
  for(i,0,rows count-1, for(j,0,maxcols-1,
    M atSet(i,j, rows at(i) at(j))
  ))
  M
)
